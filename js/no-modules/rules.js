...
// Returns [{row, col}] — destination cells for orthogonal moves.
...
function getTransformMoves(state, r, c) {
    ...
    const result = [];
    // Line: wizard cell + 3 extending cells must be empty (wizard's own cell is replaced)
    for (const [dr, dc] of DIRS) {
        const cells = [
            { r, c },
            { r: r+dr,   c: c+dc   },
    ...
    switch (move.type) {
        ...
        case 'capture': {
            ...
            if (piece.burning) {
                ...
                if (piece.power <= 0) {
                    ...
                    piece.type = BURN_LEVEL[piece.power].type;
                }
            }
            break;
        }
        ...
        case 'pyro': {
            ...
            if (burner.power <= 0) {
                ...
                burner.type = BURN_LEVEL[burner.power].type;
            }
            break;
        }
    }
